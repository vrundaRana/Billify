...
const CreateReceipt = ({ isDark = false }: CreateReceiptProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await axios.post(`${API}/api/receipts`, validatedData, {
  ...
  return (
    <main className="p-3 sm:p-6">
      ...
      <div className={`max-w-4xl mx-auto rounded-lg sm:rounded-xl shadow-xl border transition-colors duration-300 ${isDark
        ? 'bg-gray-800 border-gray-700'
        : 'bg-white border-gray-100'
        }`}>
        <div className={`px-4 sm:px-6 py-3 sm:py-4 border-b transition-colors duration-300 ${isDark ? 'border-gray-700' : 'border-gray-100'
          }`}>
          <div className="flex items-center">
            <Receipt size={20} className={`mr-2 sm:mr-3 sm:w-6 sm:h-6 ${isDark ? 'text-blue-400' : 'text-blue-600'
              }`} />
        ...
        <div className="p-4 sm:p-6">
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              <div>
                ...
                <input
                  ...
                  className={`w-full p-2 sm:p-3 rounded-lg border-2 transition-all text-sm sm:text-base ${isDark
            ...
            <div>
              ...
              <div className="space-y-3">
                {items.map((item, index) => (
                  <div key={index} className={`grid grid-cols-1 sm:grid-cols-12 gap-2 sm:gap-3 p-3 sm:p-4 rounded-lg border transition-colors ${isDark ? 'bg-gray-700/50 border-gray-600' : 'bg-gray-50 border-gray-200'
                    }`}>
                    <div className="sm:col-span-5">
                      <label className={`block text-xs font-medium mb-1 sm:hidden ${isDark ? 'text-gray-400' : 'text-gray-600'
                        }`}>Item Name</label>
                      <input
                        type="text"
                        ...
                        className={`w-full p-2 rounded border transition-all text-sm sm:text-base ${isDark
                    ...
                    <div className="sm:col-span-1 flex items-center justify-between sm:justify-center">
                      <span className={`text-sm sm:text-base font-semibold ${isDark ? 'text-green-400' : 'text-green-600'
                        }`}>
            ...
            <div className={`border-t pt-4 sm:pt-6 transition-colors ${isDark ? 'border-gray-700' : 'border-gray-200'
              }`}>
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-6 gap-2 sm:gap-0">
                <span className={`text-lg sm:text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'
                  }`}>Total Amount:</span>
                <span className={`text-xl sm:text-2xl font-bold ${isDark ? 'text-green-400' : 'text-green-600'
